...
ctx.item = function (name, parent, obj, instance, dynamic) {
	...
	* @property {Object} trackEvents */ this.trackEvents = {};
	...
	var _copyData = ['trackEvents', 'index', 'type', 'customType', 'comment', 'folder', 'occurs', 'nbOccurs', 'mustExist', 'mustNotExist'], id;
	if (obj) {
	...
	this._transcode = function (val) {
		...
		try {
			...
			if (xmlObj.Data) {
				...
				var item = (xmlObj.Data.row || xmlObj.Data.item)
...
//			});
	...
	if ((this.occurs == 0) && (!this.check) && (this._is(e.nature.EXEWIN, e.nature.WIN, e.nature.SWG, e.nature.UIAUTOMATION, e.nature.FLEX))) {
		/**
...
MyAppli.MyPage.tvTree.check(true, 'Network');
	...
	if ((this.occurs == 0) && (!this.click) && (this._is(e.nature.EXEWIN, e.nature.WIN, e.nature.WEB, e.nature.WEB3, e.nature.SWG, e.nature.UIAUTOMATION, e.nature.WINDOWLESS, e.nature.FLEX, e.nature.OCR))) {
		/**
...
MyAppli.MyPage.btSearch.clickDouble(30, 20);
    ...
    * :!: __Technology specific:__ **WIN**, **UIAUTOMATION**, **FLEX**, **OCR**
		* 
    * :!: __History:__ this function requires Contextor SDK version **3.1.1.11** minimum
    * 
		* __Ex.:__
<code javascript>
// double click on button
...
var res = MyJavaAppli.MyPage.oTree.execMethod('isSelectionEmpty()');			
	  ...
	  * @path ctx.item.execMethod
		* @param {string} method prototype of method to be executed (parameters are basic Java types : int, long, string, boolean, ... )
		* @param {string|number} [P1] parameter 1
		* @param {string|number} [P2] parameter 2
		* @param {string|number} [P3] parameter 3
		* @return {string} result value
		*/
		this.execMethod = function (method, P1, P2, P3) { //
			var desc = this.getObjectDescriptor();
			return ctx.actionApp(desc, 'execMethod', 'EXECMETHOD', desc.itemFullName, method, P1, P2, P3);
		}
	}
    ...
    * :!: __Technology specific:__ **WEB3**
		* 
    * :!: __History:__ this function requires Contextor Interactive version **3.0.4.0** minimum
		* 
    ...
    */
    this.execScript = function (list) {
	      ...
	      for (var i = 1; i < args.length; i++) {
			...
			if (func) {
	      var desc = this.getObjectDescriptor();
	      res = ctx.actionApp(desc, 'execScript', 'CALLITEM', desc.itemFullName, func, params, '', '', '3.0.4.0');
			}
			return res;
    }
	}
...
data.edName = MyAppli.MyPage.edName.get();
	...
	if (this.occurs > 0) {
   	...
   	* @param {Object} obj object to be filled with read values
		* @param {boolean} [testExist] if true, test existence before getting value
		* @return {string} result value
		*/
		var _getOccursedObject = function(item, obj, testExist) {
			var res = '';
			if (item instanceof ctx.item) {
				if (item.occurs > 1) {
					// recurse call
					for (var id in obj) {
						_getOccursedObject(item.i(id), obj[id], testExist);
					}
				} else if (item.occurs == 1) {
					// get values
					for (var index = 0; index < item.count(); index ++) {
						obj[index] = item.i(index).get(testExist);
					}
				}
			} else {
		...
		*/
		this.getAll = function (testExist) {
			...
			if (this._is(e.nature.WEB, e.nature.WEB3)) {
				...
				_getOccursedObject(this, obj, testExist);
			}
			return obj;
		}		
	}
...
var list = MyAppli.MyPage.tvList.getList());
	...
	if ((this.occurs == 0) && (!this.getProperty) && (this._is(e.nature.UIAUTOMATION, e.nature.OCR, e.nature.FLEX))) {
    /**
...
var name = MyAppli.MyPage.tvList.getProperty('Name');
    ...
    */
	...
	if ((this.occurs == 0) && (!this.getRect) && (this._is(e.nature.UIAUTOMATION, e.nature.WIN, e.nature.WEB, e.nature.WEB3, e.nature.SWG, e.nature.FLEX, e.nature.OCR))) {
    /**
...
// ["", "0000937", "4", "smith", "ford"], 
	...
	if ((!this.highlight) && (this._is(e.nature.UIAUTOMATION, e.nature.WIN, e.nature.WEB, e.nature.WEB3, e.nature.SWG, e.nature.FLEX, e.nature.OCR))) {
		if (this.occurs > 0) {
			/**
	  ...
	  * @path ctx.item.i
		...
		*/
		this.i = function (arg) {
			...
			return new ctx.item(this.name, this.parent, { index: index, occurs: (this.occurs - args.length), mustExist: this.mustExist, mustNotExist: this.mustNotExist, type: this.type } );
		}
...
if (MyAppli.MyPage.btSearch.isVisible()) { ... }
	...
	if ((this.occurs == 0) && (!this.keyStroke) && (this._is(e.nature.WEB, e.nature.WEB3, e.nature.SWG, e.nature.UIAUTOMATION, e.nature.OCR, e.nature.FLEX))) {
		/**
...
MyAppli.MyPage.edSearch.keyStroke(e.key.Enter);
	  ...
	  * @path ctx.item.keyStroke
		...
		*/
		this.keyStroke = function (command) {
			...
			if (this._is(e.nature.UIAUTOMATION, e.nature.OCR, e.nature.FLEX))
	...
	if ((this.occurs == 0) && (!this.keyStroke2) && (this._is(e.nature.UIAUTOMATION, e.nature.OCR, e.nature.SWG))) {
		/**
...
MyAppli.MyPage.MyItem.set(data.edName);
	  ...
	  * @path ctx.item.set
		...
		* @return {string} result value
		*/
		this.set = function (value, testExist) {
			var desc = this.getObjectDescriptor();
			if ((typeof value === 'undefined') || (testExist && (!this.exist())))
				return '';
	...
	if (this.occurs > 0) {
   	...
   	* @param {Object} obj object contining values to be set
		...
		* @return {string} result value
		*/
		var _setOccursedObject = function(item, obj, testExist) {
			var res = '';
			if (item instanceof ctx.item) {
				if (item.occurs > 1) {
					// recurse call
					for (var id in obj) {
						_setOccursedObject(item.i(id), obj[id], testExist)
					}
				} else if (item.occurs == 1) {
					// set end values
					for (var id in obj) {
						res = item.i(id).set(obj[id], testExist);
					}
				}
			} else {
		    throw new Error(e.error.InvalidArgument, 'ctx.item.setAll: Invalid object');				
