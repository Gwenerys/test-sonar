...
ctx.tooltipClass = function (params, page) {
	...
	/** @type {string} */ var _rootRegistry = "Software\\Contextor\\CtxtRun\\Settings\\Tooltips\\";
	...
	* @property {string} */ this.ctxType = 'ctx.tooltipClass';
	...
	/** Clones a given tooltip and attach it to a page [internal use]
	* __Ex.:__
	<code javascript>
  ...
  * @path ctx.tooltipClass.set
	...
	*/
	this.set = function (params) {
		...
		if (params) {
			var _params = {};
			var lib = params.library || _page.tooltipOptions.library || ctx.options.tooltips.library || e.tooltip.library.tooltipster;
		...
		//							}							
				...
				if (_page.tooltipOptions.library == e.tooltip.library.opentip) {
					// *** Opentip tooltip initialization ***
					var text = _tooltip.local.content || "";
					//res = _page.execScript("ctxTooltip.map['" + local.id + "'] = new Opentip", "#" + tooltip.local.targetId, text, tooltip.options, e.prefix.tryCatch);
					_page.execScript("ctxTooltip.create", _tooltip.local.id, _tooltip.local.targetId, text, _tooltip.options);
					if ((!text) && _tooltip.local.helpId) { 
...
ctx.page = function (name, parent, obj, instance) {
  ...
  /** page model (root) object
	...
	* @ignore
  * @type {Array.<string>} */ var _copyData = [/*'tooltips',*/ 'tooltipOptions', 'trackEvents', 'path', 'pathList', 'comment', 'nature', 'data'];
	if (obj) {
		var id;
	...
	* @ignore
	* @path ctx.page._is
	* @param {...string} arg list of natures (ex.: 'WEB', 'WEB3', 'WIN', ...) (see [[lib:common:ctx.enum#enature|e.nature]])
...
<page>.addOn({ event: function(ev) {
  ...
  * @param {number} [delay] optional delay to wait before calling the callback (default is 0)
  ...
  this.addOn = function (evCallback, immediateCondition, single, delay, noStepReset) {
	...
	* @description
	* The list of pre-defined events is defined in enumeration : [[lib:common:ctx.enum#eeventpage|e.event.page]]
	* 
	* __Ex.:__
<code javascript>
...
MyAppli.MyPage.addEvent({evMyFuncEvent_1:'', evMyFuncEvent_2:''});
  ...
  */
    ...
    for (var name in obj) {
//			if ((name.substring(0, 1) == '$') || (name.substring(0, 1) == '_'))
//				return new ctx.event(name, this); // don't store internal tech. events
      if (!this.events[name]) {
        this.events[name] = new ctx.event((obj[name] ? obj[name] : name), this, {}, technical);
				if (addOnParent && this.parent && this.parent.events) {
	        this.parent.events[name] = new ctx.event((obj[name] ? obj[name] : name), this, {}, technical);
				}
      }
      ev = this.events[name];
    }
  ...
  */
	this.addItem = function (name, obj, dynamic) {
		if ((!this.items[name]) && this[name]) {
			throw new Error(e.error.ReservedId, this.parent.name + "." + this.name + " : '" + name + "' item can't be added: name is already used or reserved");
		}
...
//			try {
//				// delete objects inserted by Contextor
//				res = page.deleteObject(ctx.objectClass, e.html.type.className); 
//			} catch (ex) { }
		}
		return res;
  }
...
var localFile = ctx.options.currentDir + "\\bill.pdf";
    ...
    */
    this.download = function (params) {
	      ...
	      throw new Error(e.error.InvalidArgument, 'ctx.page.download: url and localFile should be defined');
			...
			this._initAjax();
			var desc = GLOBAL.events._evFileReception.getObjectDescriptor().ctxShort();
			params.desc = desc;
...
MyWebAppli.MyPage.execScript('SendAnswerToHub', req.info.destination.id, req.ev);		
    ...
    */
    this.execScript = function (list) {
	      ...
	      if (typeof code !== "string") throw new Error(e.error.InvalidArgument, 'ctx.page.execScript: Invalid argument 0 (should be a string)');
	      for (var i = 1; i < args.length; i++) {
					if (args[i] === e.prefix.tryCatch) {
						useTryCatch = true;
					} else {
...
if (MyAppli.MyPage.exist()) { ... }
  ...
  */
    ...
    var desc = this.getObjectDescriptor();
		...
		//return false;
//    /*var test = ctx.verbExec(desc, 'exist', 'EXIST', {
//      Page: desc.pageName
//    }, '<SETVALUE ZoneCtx="_Work0_" Value="1"/>', true);
//    res = (test == '1' ? true : false);*/
		return res;
	}
...
// obj = {x:68, y:157, cx:350, cy:32}
    ...
    */
    this.getFocusRect = function () {
			var desc = this.getObjectDescriptor();
			var str = ctx.actionApp(desc, 'getFocusRect', 'GETWINFOCUSRECT', '', '', '', '', '', '3.0.6.3');
...
// </Menu>
  ...
  */
    ...
    if (object.parent) {
      ...
      if (object.name !== '') {
        desc.pageName = object.name;
      } else if (ctx.currentEvent) {
        desc.pageName = ctx.currentEvent.pageName;
      }

      if (object.instance != -1)
        desc.pageInst = object.instance; // instance is explicitly mentioned
      else if (ctx.currentEvent && (desc.appliName == ctx.currentEvent.appliName) && (desc.pageName == ctx.currentEvent.pageName))
...
var path = MyAppli.MyPage.getPath();
  ...
  * @param {e.env} [env] environment ('dev', 'prod', ...). If omitted, 'ctx.options.env' is used. See '[[lib:common:ctx.enum#eenv|e.env]]' for more details
  ...
  */
  ...
  if (this._is(e.nature.UIAUTOMATION, e.nature.WIN, e.nature.WEB, e.nature.WEB3, e.nature.SWG, e.nature.FLEX, e.nature.OCR)) {
    /**
...
// obj = {x:68, y:157, cx:120, cy:32}
    ...
    */
    this.getRect = function (desc) {
			...
			ctx.noNotify = true;
			var str = ctx.actionApp(desc, 'getRect', 'GETRECT', desc.itemFullName, '', '', '', '', '3.0.6.6');
			var obj = ctx.unserialize(str, true);
			/** @type {ctx.position} */ var rect = new ctx.position();
			ctx.set(obj, rect);
			return rect;
      ...
      for (var i in args) {
        var func = args[i];
				if ((typeof func === 'function') && (func.toString)) {
					var funcContent = func.toString();
					var funcName = funcContent.match(/function\s+([^\s\(]+)/);
					if (funcName && funcName[1]) {
						code = code + "if ('undefined' === typeof(" + funcName[1] + ")) { " + funcContent + " };\n";
					}
				} else {
...
MyWebAppli.MyPage.execScript('create_iframe();');
    ...
    * @param {e.scriptLanguage} [language] JavaScript or VBScript language (see [[lib:common:ctx.enum#escriptlanguage|e.scriptLanguage]]) (JavaScript  by default)
    * @return {string} result value
    */
  ...
  "style":"width:260;height:24"
	...
	MyWebAppli.events.evOnButton2
	  ...
	  * @path ctx.page.insertButton
		...
		* @param {e.html.position} [position] insert position relative to 'adjacentItem' (default is 'e.html.position.afterEnd'). See [[lib:common:ctx.enum#ehtmlposition|e.html.position]]
...
res = MyWebAppli.MyPage.insertObject('input', MyWebAppli.MyPage.MyItem, {
  ...
  e.html.position.afterEnd, 
  MyAppli.events.evOnMyNewButtonClicked
);
</code>
		* @method insertObject
		 ...
		 this.insertObject = function (tag, adjacentItem, parameters, position, event, disableTimer) {
			...
			if (this.exist())
			{
				...
				if (event && (event instanceof ctx.event)) {
					...
					parameters.onclick = "Contextor.Event( '" + event.name + "', '" + appliName + "', '" + pageName + "', '', " + appliInst + ", " + pageInst + ", '" + id  + "');";
					if (disableTimer > 0) {
	  ...
	  * @path ctx.page.tooltipInit
		...
		*/
		this.tooltipInit = function (params) {
			...
			//if (ctx.options.tooltips.enabled && _page.exist()) {
				try {
					...
					if (thePage.tooltipOptions.library == e.tooltip.library.opentip) {
						...
						ccsCode = ctx.fso.file.read(ctx.options.currentURL + "\\opentip\\opentip.css") + "\n\n";
						ccsCode += ctx.fso.file.read(ctx.options.currentURL + "\\opentip\\ctx.opentip.css") + "\n\n";
					} else {
						// *** Tooltipster CSS initialization ***
						// insert a style block for tooltip CSS
						ccsCode = ctx.fso.file.read(ctx.options.currentURL + "\\tooltip\\tooltipster.bundle.min.css") + "\n\n";
						//switch (thePage.tooltipOptions.theme) {
						//	case e.tooltip.theme.light:
								ccsCode += ctx.fso.file.read(ctx.options.currentURL + "\\tooltip\\tooltipster.light.css") + "\n\n";
						//		break;
						//	case e.tooltip.theme.shadow:
								ccsCode += ctx.fso.file.read(ctx.options.currentURL + "\\tooltip\\tooltipster.shadow.css") + "\n\n";
						//		break;
						//	case e.tooltip.theme.white:
								ccsCode += ctx.fso.file.read(ctx.options.currentURL + "\\tooltip\\tooltipster.white.css") + "\n\n";
						//		break;
						//}
						ccsCode += ctx.fso.file.read(ctx.options.currentURL + "\\tooltip\\tooltipster.custom.css") + "\n\n";
					}
					...
					if (thePage.tooltipOptions.css) {
						...
						ctx.each(thePage.tooltipOptions.css, function(id, value) {
							ccsCode += ctx.fso.file.read(ctx.options.currentURL + "\\" + value) + "\n\n";
						});
					}
					...
					if (thePage.tooltipOptions.library == e.tooltip.library.opentip) {
						...
						if (thePage.tooltipOptions.jQuery) {
							...
							if (jQueryInject) {
								thePage.injectFile(ctx.options.currentURL + "\\opentip\\jQuery" + minSuffix + ".js", 'ctxTooltipJQuery', true);
							}
							thePage.injectFile([ctx.options.currentURL + "\\opentip\\opentip-jquery" + minSuffix + ".js", ctx.options.currentURL + "\\opentip\\excanvas.js"], 'ctxTooltipLib', true);
							//thePage.injectFile(ctx.options.currentURL + "\\opentip\\excanvas.js", 'ExcanvasLib', true);
							//if ((jQueryInject) && (thePage.tooltipOptions.noConflict)) {
							if (jQueryInject) {
								thePage.execScript("jQuery.noConflict();");								
							}
						} else {
							thePage.injectFile([ctx.options.currentURL + "\\opentip\\opentip-native" + minSuffix + ".js", ctx.options.currentURL + "\\opentip\\excanvas.js"], 'ctxTooltipLib', true);
							//thePage.injectFile(ctx.options.currentURL + "\\opentip\\excanvas.js", 'ExcanvasLib', true);
						}
						thePage.injectFile(ctx.options.currentURL + "\\opentip\\ctx.opentip.js", 'ctxMyTooltipLib', true);
						...
						if (jQueryInject || thePage.tooltipOptions.jQuery) {
							thePage.injectFile(ctx.options.currentURL + "\\tooltip\\jQuery" + minSuffix + ".js", 'ctxTooltipJQuery', true);
						}
						thePage.injectFile(ctx.options.currentURL + "\\tooltip\\tooltipster.bundle" + minSuffix + ".js", 'ctxTooltipLib', true);
						thePage.injectFile(ctx.options.currentURL + "\\tooltip\\ctx.tooltip.js", 'ctxMyTooltipLib', true);
					}
					...
					if (thePage.tooltipOptions.file) {
						...
						ctx.each(thePage.tooltipOptions.file, function(id, value) {
							htmlCode += ctx.resources.loadHtml(ctx.options.currentURL + '\\' + value);
							htmlCode += "\n\n";
						});
						var htmlObject = {
...
//				});
			...
			//_page.waitClose(function(ev) {
			//_page.waitCloseAll(function(ev) {
			//_page.model.events.UNLOAD.on(function(ev) {
			// clean up when quitting Contextor
...
var txt = MyWinAppli.MyPage.isEnabled();	
	...
	if (this._is(e.nature.WIN, e.nature.EXEWIN, e.nature.UIAUTOMATION, e.nature.OCR, e.nature.HLLAPI)) {
		...
		* @description
		* The list of pre-defined keys is defined in enumeration : [[lib:common:ctx.enum#ekey|e.key]]
		* 
...
MyAppli.MyPage.keyStroke(e.key.Alt + e.key.F7);	
		...
		* @param {string} command key sequence or text to be sent (see [[lib:common:ctx.enum#ekey|e.key]])
		* @return {string} result value
		*/
    ...
    /**
		...
		* @description
		* The list of pre-defined keys if defined in enumeration : [[lib:common:ctx.enum#ekey|e.key]]
		* 
...
MyWebAppli.MyPage.navigate('http://www....', e.targetFrame.Blank);
    ...
    * @param {e.targetFrame} [dest] expression that evaluates to the name of the frame in which to display the resource (see [[lib:common:ctx.enum#etargetframe|e.targetFrame]]).
...
MyUIAAppli.MyPage.refresh();
    ...
    */
	...
	{
		/**
		* Refresh asynchrone the page cache from the running application
		* @description
...
MyAppli.MyPage.restore();
  ...
  * Generates a screenshot
	...
	* __Ex.:__
<code javascript>
MyAppli.MyPage.screenshot(ctx.options.currentDir + '\\MyAppli.png');
</code>
...
MyAppli.MyPage.setItems(data);
	 ...
	 * @method setItems
	 * @path ctx.siebel.setItems
	 * @param {Object} obj object with data to be set in page
...
MyAppli.MyPage.start();
  ...
  * @param {e.env} env environment ('dev', 'prod', ...). See '[[lib:common:ctx.enum#eenv|e.env]]' for more details
  ...
  if (this._is(e.nature.WIN, e.nature.WEB, e.nature.WEB3)) {
    ...
    * :!: __Technology specific:__ **WIN**, **WEB**, **WEB3**
	...
	MyAppli.MyPage.setWindowStyle(e.windowStyle.MaximizeBox, true);	
	  ...
	  * @param {e.windowStyle} style window style (see [[lib:common:ctx.enum#ewindowstyle|e.windowStyle]])
...
MyWebAppli.MyPage.setVisible(false, e.windowBarType.AddressBar);
  ...
  * @param {e.windowBarType} [barType] element of the Web browser to be shown/hidden (see [[lib:common:ctx.enum#ewindowbartype|e.windowBarType]]).\\ If omitted, the full page is shown/hidden.\\ This parameter is only significative for WEB technology.
	* @return {string} result value
  */
...
MyAppli.MyPage.start("http://www.dokuwiki.org/");
  ...
  * @param {e.launchFlag} [flag=e.launchFlag.Show] launch flag. See [[lib:common:ctx.enum#elaunchflag|e.launchFlag]] for more details
  ...
  */	
    ...
    if (bEvenIfExist || (!this.exist())) {
			...
			if (parm) { parm = ctx.resolvePath(parm); }
			if (path) { path = ctx.resolvePath(path); }
...
<page>.waitClose(function(ev) {
  ...
  */
  ...
  this.addEvent({ ACTIVATE: '', LOAD: '', UNLOAD: '' }, true, true);
	if (this._is(e.nature.EXEWIN, e.nature.WIN)) {
