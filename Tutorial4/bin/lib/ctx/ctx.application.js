...
* \\
*
* The ''ctx.application'' class implements the applications and processes declared in the project, especially to perform actions : start, close, ...
*
...
...
// notify this event
<...>.notify(<application>.events.<event>, data);
</code>
* 
* __Ex.:__
<code javascript>
...
ctx.application = function (name, obj) {
  ...
  /** class type
	...
	* @property {ctx.dataManager} */ this.dataInternal = new ctx.dataManager(null);

	/** data template container
	* @path ctx.application.dataTemplates
	* @property {ctx.dataManager} */ this.dataTemplates = new ctx.dataManager(null);
  ...
  /** application model (root) object
	...
	* @property {string} */ this.navigator = '';
...
//	* @property {Object} */ this.requests = {};
  ...
  var _copyData = ['scenarios', 'steps', 'navigator', 'trackEvents', 'instance', 'path', 'pathList', 'comment', 'nature', 'labels', 'dataTemplates', 'functions'], id;
  if (obj) {
  ...
  this.getObjectDescriptor = function (desc) {
    ...
    desc.nature = object.nature || desc.nature;
		...
		if (object.instance != -1) {
			...
			desc.appliInst = object.instance;
		} else if (ctx.currentStep && ((scInst = ctx.currentStep.sc.appliInst[desc.appliName]) > 0)) {
			// is there a running scenario with a default instance defined ?
			desc.appliInst = scInst;
		} else if (ctx.currentStep && ctx.currentStep.sc && ctx.currentStep.sc.parent && (desc.appliName == ctx.currentStep.sc.parent.name) && (ctx.currentStep.sc.parent.instance >= 0) && ctx.app[ctx.currentStep.sc.parent.name] && ctx.app[ctx.currentStep.sc.parent.name][ctx.currentStep.sc.parent.instance]) {
			// is there a running scenario whose parent is this application ?
			desc.appliInst = ctx.currentStep.sc.parent.instance;
		} else if (ctx.currentEvent && (desc.appliName == ctx.currentEvent.appliName) && (ctx.currentEvent.appliInst > 0) && ctx.app[ctx.currentEvent.appliName] && ctx.app[ctx.currentEvent.appliName][ctx.currentEvent.appliInst]) {
			// the object is the current application
			desc.appliInst = ctx.currentEvent.appliInst;
		} else if (this.defaultInst > 0) {
			// is there a default instance to be used for this application ? 
			desc.appliInst = this.defaultInst;
...
//				return new ctx.event(name, this);
      if (!this.events[name]) {
				var data;
				if (typeof obj[name] === 'object') { data = obj[name]; }
...
<application>.addOn({ event: function(ev) {
  ...
  * @param {number} [delay] optional delay to wait before calling the callback (default is 0)
  ...
  this.addOn = function (evCallback, immediateCondition, single, delay, noStepReset) {
	...
	* The list of pre-defined technical events is defined in enumeration : [[lib:common:ctx.enum#eeventapplication|e.event.application]]
	* 
	* __Ex.:__
	* <code javascript>
  ...
  */
  this.addPage = function (pageName, obj, pageInst) {
    if ((!pageInst) && (!this.pages[pageName]) && this[pageName]) {
      throw new Error(e.error.ReservedId, this.pageName + " : '" + pageName + "' page can't be added: name is already used or reserved");
    }
  ...
  */
	...
	if (this._is( e.nature.HLLAPI )) {
	    ...
	    this.addOn({ evHLLAPIConnect: function(ev) {
				if (!this.dataInternal.isHllConnected) {
						try {
							this[0].technicalPage.connectPS(this.dataInternal.session, this.dataInternal.dll, this.dataInternal.api);
							this.dataInternal.isHllConnected = true;
							if (this.nbInst == 1)
							{
							...
							if (this.dataInternal.connectFunc) {
								this.dataInternal.connectFunc(false); // failure
								this.dataInternal.connectFunc = undefined;
							}
						}
				}
				if (this.dataInternal.isHllConnected && this.dataInternal.connectFunc) {
					this.dataInternal.connectFunc(true); // connected : success
					this.dataInternal.connectFunc = undefined;
				}
			}});
			...
			this.pageDisconnect = function () {
				if (this.dataInternal.isHllConnected) {
					try {
						...
						this[0].technicalPage.disconnectPS();
						this.dataInternal.isHllConnected = false;
					} catch (ex) {	
						ctx.log(ex, e.logIconType.Error, 'ctx.page.disconnectPS failed :')
						if (this.dataInternal.disconnectFunc) {
							this.dataInternal.disconnectFunc(false); // failure
							this.dataInternal.disconnectFunc = undefined;
						}
					}
				}
				if (!this.dataInternal.isHllConnected && this.dataInternal.disconnectFunc) {
					this.dataInternal.disconnectFunc(true); // disconnected : success
					this.dataInternal.disconnectFunc = undefined;
				}			
			}
    ...
    */
    this.connect = function (session, func, dll, api) {
	      ...
	      throw new Error(e.error.InvalidCommand, this.name + ".connect() failed : no technical page available");
			...
			GLOBAL.notify(this.events['evHLLAPIConnect']);
    }
	}
...
_page = _parentProcess.createExtendedConnector(e.extendedConnector.ExpBar, _name);
  ...
  * @param {e.extendedConnector} id connector name (COM object id of the component)\\ (see [[lib:common:ctx.enum#eextendedconnector|e.extendedConnector]])
    ...
    */
    this.disconnect = function (func) {
	      ...
	      throw new Error(e.error.InvalidCommand, this.name + ".connect() failed : no technical page available");
			...
			GLOBAL.notify(this.events['evHLLAPIDisconnect']);
    }
	}
...
MyAppli.notify(MyAppli.events.evLogin, obj, 5000);
  ...
  */
  this.notify = function (event, data, timer, appliInst, method, itemName, reqEvent) {
    var desc = this.getObjectDescriptor();
    var targetDesc = ((event instanceof ctx.event) ? event.getObjectDescriptor() : desc );
    var _appliInst = (appliInst !== undefined ? parseInt(appliInst, 10) : ((targetDesc.appliInst == -1) ? undefined : targetDesc.appliInst));
    ...
    return ctx.verbExec(desc, 'notify', 'NOTIFY', {
      Event: ((event instanceof ctx.event) ? event.name : event),
      Appli: targetDesc.appliName,
      InstanceAppli: ((_appliInst == -1) ? 'All' : _appliInst),
      Page: targetDesc.pageName,
      InstancePage: pageInst,
      Timer: timer,
      Data: data,
      Method: method,
			EventResp: ((reqEvent && (reqEvent instanceof ctx.event) && reqEvent.name) ? reqEvent.name : ((event instanceof ctx.event) ? event.reqEventName : undefined ))
    });
  }
  ...
  */
  this.removePage = function (appliInst, pageName, pageInst) {
		...
		if (this[appliInst]) {
			if (this[appliInst][pageName]) {
				if (this[appliInst][pageName][pageInst]) {
...
<application>.scenario({ sc: function(ev, sc) {
  ...
  */
  this.scenario = function (obj, dataClass, comment) {
		...
		if (typeof obj === 'object') {
		  for (var scName in obj) {
			    ...
			    if (!this.scenarios[scName]) {
			      this.scenarios[scName] = sc = new ctx.scenarioClass(scName, fn, this, false, dataClass); // create new
...
MyJavaAppli.captureApplicationTree(hwnd, '....xml', '0|0|0|0');
    ...
    */
		...
		* @description
		* 
	  * :!: __History:__ this function requires Contextor Interactive version **3.0.6.8** minimum
		*
		* __Ex.:__
	<code javascript>
	</code>
		...
		*/
		this.captureApplicationTree = function(hwnd, filename, position) {
			...
			if (pg && pg.exist()) {
				...
				try {
		      var desc = pg.getObjectDescriptor();
		      res = ctx.actionApp(desc, 'captureApplicationTree', 'CAPTURE', handle, position, '', '', '', '3.0.6.8');
					data = {
						success: true,
						//error: 0,
						data: ''
		...
		if (typeof obj === 'object') {
		  for (var stName in obj) {
			    ...
			    if (!this.steps[stName]) {
						...
						throw new Error(e.error.DuplicateId, name + '.step: name already used: ' + stName);
					}
				}
		  }
		}
...
MyAppli.waitPages(function(ev) {
  ...
  */
  ...
  if (this.nature != '') {
	...
	// *** delete items dynamically declared in pages when Contextor terminates ***
	var appli = this;
...
// specific events
GLOBAL.addEvent( { UPDATECTX:'', PRESTOPCTX:'', STOPCTX:'', RESTARTCTX:'', _evStopRequest: '', _evStopBeingHandled: '', _evStopDone: '', _evUpdateLogFolder: '' });
...
GLOBAL.addOn({ _evFileReception: function(ev) {
	var content, options;
...
GLOBAL.events.PRESTOPCTX.on(function(ev) {
	var nbListeners = 0;
	GLOBAL.notify(GLOBAL.events._evStopRequest);
	var hObj = GLOBAL.events._evStopBeingHandled.on(function(ev) {
		nbListeners ++;
	});
	var hObjDone = GLOBAL.events._evStopDone.on(function(ev) {
		nbListeners --;
		if (nbListeners == 0) {
...
*/
var sc = new ctx.scenarioClass('scDummy', function(ev, sc) {}, GLOBAL, true);
/** 
* @path st
* @type {ctx.stepClass}
*/
...
/** 
* @path req
* @type {ctx.requestClass}
*/
var req = new ctx.requestClass({}, 'evDummy');
/** 
* @path desc
* @type {ctx.descriptor}
*/
var desc = new ctx.descriptor();
/** 
* @path rect
* @type {ctx.position}
*/
